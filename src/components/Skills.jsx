...
const Skills = () => {
  ...
  const ProgressBar = ({ level, delay, isActive }) => {
    ...
    return (
      <div className="relative h-1 overflow-hidden rounded-full bg-neutral-800">
        ...
        />
        <motion.span
          className="absolute right-0 text-xs font-medium text-teal-400 -top-6"
          initial={{ opacity: 0 }}
  ...
  return (
    <section ref={ref} id="skills" className="relative px-4 py-16 overflow-hidden sm:px-6 md:py-32 md:px-8 bg-neutral-950">
      {/* Subtle background */}
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
          {skillCategories.map((category, index) => (
            ...
            >
              <motion.div 
                className="h-full p-6 transition-all border duration-400 rounded-2xl border-neutral-800 bg-neutral-900/30 hover:border-teal-500/20 hover:bg-neutral-900/50 group hover:shadow-lg hover:shadow-teal-500/5"
                style={{ transitionTimingFunction: 'cubic-bezier(0.16, 1, 0.3, 1)' }}
              >
                {/* Category Header */}
                <div className="flex items-center gap-3 mb-6">
                  <motion.div 
                    className="p-2.5 rounded-xl bg-neutral-800 text-teal-400 group-hover:bg-teal-500/10 transition-colors duration-300"
                    whileHover={{ scale: 1.1 }}
                    transition={{ duration: 0.2, ease: premiumEase }}
                  >
                    {category.icon}
                  </motion.div>
                  <h3 className="text-xl text-white font-editorial-regular">
                    {category.title}
                  </h3>
                </div>
                  ...
                  {category.skills.map((skill, skillIndex) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm transition-colors duration-300 text-neutral-300 group-hover/skill:text-teal-400">
                          {skill.name}
                        </span>
                        <span className="text-xs text-neutral-500 px-2 py-0.5 rounded-full bg-neutral-800 group-hover/skill:bg-teal-500/10 group-hover/skill:text-teal-400 transition-all duration-300">
                          {skill.level}
                        </span>
                      </div>
                      <ProgressBar 
